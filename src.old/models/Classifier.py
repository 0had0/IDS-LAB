...
class Softmax(nn.Module):
    ...
    def __init__(self, input_dim=76, num_classes=14, device=DEVICE):
...
class   Classifier:
    ...
    def __init__(
        ...
        lr=1e-3,
        reg=1e-5,
        runs_dir="models/Classifier_logs",
        seed=10,
    ):
        ...
        self.reg = reg
        self.runs_dir = runs_dir + key
        self.seed = seed
        self.key = key
        # self.device = 'cuda'
        self.logger = logging.getLogger(self.key)
        self.logger.setLevel(logging.DEBUG)
        file_handler = logging.FileHandler(f"training-{key}.log")
        ...
        self.logger.info(
            "Classifier initialized with method %s, input_dim %d, num_classes %d, num_epochs %d, batch_size %d, lr %f, reg %f, runs_dir %s"
            % (
                method,
                input_dim,
                num_classes,
        ...
        if method == Method.SOFTMAX.value:
            ...
            self.model = Softmax(
                input_dim, num_classes=num_classes, device=self.device
            )
            ...
            raise ValueError(
                "Method must be one of 'softmax', 'cnn2', 'cnn5', 'nn3', or 'nn5'."
            )
    ...
    def fit(self, X, Y):
        ...
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=0.1, random_state=self.seed
        )
        ...
        tensor_x = torch.stack([torch.Tensor(i) for i in X_dev]).to(
            self.device
        )
        tensor_y = torch.LongTensor(Y_dev).to(
            self.device
        ...
        for epoch in range(resume_epoch, num_epochs):
            for i, (xi, yi) in enumerate(train_loader):
                ...
                loss.requires_grad
                seen_so_far = self.batch_size * (
                    epoch * len(train_loader) + i + 1
                )  # fixes issues with different batch size
                ...
                if (seen_so_far / self.batch_size) % 50 == 0:
                    pred = self.predict(X_val)
                    balanced_acc = (
                        metrics.balanced_accuracy_score(Y_val, pred) * 100
                    )
                    ...
                    self.logger.debug(
                        ...
                        num_epochs,
                        i + 1,
                        len(Y_dev) // self.batch_size,
                        loss.item(),
                    )

                    writer.add_scalar(
                        "Accuracy/Balanced Val", balanced_acc, seen_so_far
                    )
    ...
    def load_checkpoint(self, filepath):
        if os.path.isfile(filepath):
            ...
            print(
                "Loaded {} model trained with batch_size = {}, seen {} epochs and {} mini batches".format(
                    self.runs_dir,
